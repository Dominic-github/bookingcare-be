...
let getParkageBySelectionCategory = async (req, res) => {
  try {
    let info = await parkageService.getParkageBySelectionCategory(
...
let getParkageBySelectionPrice = async (req, res) => {
  try {
    let info = await parkageService.getParkageBySelectionPrice(
      req.query.priceId
    )
    return res.status(200).json(info)
  } catch (error) {
...
let getParkageBySelectionClinic = async (req, res) => {
  try {
    let info = await parkageService.getParkageBySelectionClinic(
      req.query.clinicId
    )
    return res.status(200).json(info)
  } catch (error) {
...
let getListPatientParkage = async (req, res) => {
  try {
    let data = await parkageService.getListPatientParkage(
      req.query.parkageId,
      req.query.date
    )
