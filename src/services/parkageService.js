  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.clinicId || !data.specialtyId || !data.name) {
        resolve({
          errCode: 1,
          errorMessage: 'Misssing parameter'
        })
      } else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!parkage) {
        resolve({
          errCode: 1,
          errorMessage: 'parkage does not exist'
        })
      } else {
        ...
        resolve({
          errCode: 0,
          errorMessage: 'Delete parkage success'
        })
      }
    } catch (error) {
      reject(error)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!q) {
        resolve({
          errCode: 1,
          errorMessage: 'Missing querry search'
        })
      } else {
        let data = await db.Parkages.findAll({
          where: {
